...
if dir_out is None and LOAD == "load":
    ...
    dir_out = data_dir+'/'+f"load_resDir-{os.path.basename(dir_resume)}_seed-{str(SEED)}"+'/'
...
if init == "cold":
    ...
    res_filename = os.path.join(dir_resume, "res")
    
    psic_1, psic_2, qc_1, qc_2, mc, t0 = qg_io_3d.load_res_file(  res_filename )
    
    if model == "moist":
        
        ## loading files from the resume directory, then saving to new directory
        ds_res, zu1, zu2, ztau, zeke1, zeke2, zemf1, zemf2, zehf1, zehf2, zm, zP, zE, zw, zwskew, time = qg_io_3d.load_moist_data( filename_res)
        ds3_res, u1n, u2n, v1n, v2n, taun, q1n, q2n, mn, Pn, En, wn, psi1n, psi2n = qg_io_3d.load_moist_data_xyt( filename3_res)
        ...
        for v in list(ds.variables):
            ds.variables[v][:] = 0.0
            var_shape = ds_res.variables[v][:].shape
            ds.variables[v][:][0] = ds_res.variables[v][-1]
        ds.sync()
        print(ds_res.variables)
        print(ds_res.variables["zm"][:20])
        print(ds_res.variables["zm"].shape)
        print(ds.variables["zm"][:20])
        print(ds.variables["zm"].shape)
        
        for v in list(ds3.variables):
            var_shape = ds3_res.variables[v][:].shape
            ds3.variables[v][:][:var_shape[0]] = ds3_res.variables[v][-1]
        ds3.sync()
        
        # print(ds.variables["mn"].shape)print(ds3.variables["psi1"][:])
        print(ds3_res.variables["psi1"][-5:])
        print(ds3_res.variables["psi1"].shape)
        print(np.std(ds3_res.variables["psi1"][:var_shape[0]]))
        print(np.std(ds3_res.variables["psi1"][var_shape[0]:]))
        
        print(ds3.variables["psi1"][:5])
        print(ds3.variables["psi1"].shape)
        print(np.std(ds3.variables["psi1"][:var_shape[0]]))
        print(np.std(ds3.variables["psi1"][var_shape[0]:]))
        raise
    ...
    # else:
         # ds, zu1, zu2, ztau, zeke1, zeke2, zemf1, zemf2, zehf1, zehf2, time = qg_io_3d.load_dry_data( filename )
         # ds3, u1n, u2n, v1n, v2n, taun, q1n, q2n, mn, Pn, En, wn, psi1n, psi2n, time = qg_io_3d.load_dry_data_xyt( filename3, x, y, int(tot_time - lim))
    ...
    # nfact = .1
    # if NOISE == "gauss":
      # psi_1_std = np.std(psi_1)
      # psi_2_std = np.std(psi_2)
