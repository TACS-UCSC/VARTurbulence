...
data_dir = "/glade/derecho/scratch/llupinji/data/qgm_sim"
...
parser.add_argument("-d", "--dir_out", default = None, help="output directory, if specified, will override the name and seed.", type=str)
...
parser.add_argument("-tt", "--tot_time", default = 20000, help="total time of the simulation", type=int)

parser.add_argument("-dt", "--dt", default = 0.025, help="timestep of the simulation", type=float)

parser.add_argument("--name", default = "qgm_sim", help="name of the simulation", type=str)
...
if dir_out is None and LOAD == "cold":
    dir_out = os.path.join(DATA_DIR, f"{NAME}_seed-{str(SEED)}")
if dir_out is None and LOAD == "load":
    ...
    dir_out = os.path.join(DATA_DIR, f"load_resDir-{os.path.basename(dir_resume)}_noise-{NOISE}_seed-{str(SEED)}")
...
filename = os.path.join(dir_out, "output.2d.nc")
filename3 = os.path.join(dir_out, "output.3d.nc")
res_filename = os.path.join(dir_out, "res")
res_filename_init = os.path.join(dir_out, "res_init")
...
# domain
N = 256 #zonal size of spectral decomposition
N2 = 256 #meridional size of spectral decomposition
...
with open(os.path.join(dir_out, "parameters.pkl"), "wb") as h:
