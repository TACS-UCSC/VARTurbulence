...
class Quantizer2d(eqx.Module):
    ...
    def __call__(self, x):
        ...
        return z_q, self.codebook_updates(flatten, codebook_indices), indices
    ...
    def codebook_updates(self, flatten, codebook_indices):
        ...
        # EMA update for codebook average
        new_codebook_avg = (
...
class VARVQVAE2d(eqx.Module):
    """VAR-style multi-scale VQ-VAE for 2D turbulence data."""
    encoder: Encoder2d
    decoder: Decoder2d
...
class VQVAE2d(eqx.Module):
    """VQ-VAE for 2D turbulence data."""
    encoder: Encoder2d
    decoder: Decoder2d
    quantizer: Quantizer2d
    ...
    def decode_indices(self, indices):
        ...
        # indices shape: [H', W']
        H, W = indices.shape
        z_q = self.quantizer.codebook[indices.flatten()]  # [H'*W', D]
        z_q = jnp.reshape(z_q, (H, W, self.quantizer.D))  # [H', W', D]
        z_q = jnp.transpose(z_q, (2, 0, 1))  # [D, H', W']
        return self.decoder(z_q)
